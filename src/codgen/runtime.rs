...
impl<'ctx> Runtime<'ctx> {
    pub fn new(context: &'ctx inkwell::context::Context, module: &Module<'ctx>) -> Self {
        ...
        let print = module.get_function("print_value").expect("runtime to include print");
        let get_float = module.get_function("get_float").expect("runtime to include get_float");
        let next_line = module.get_function("next_line").expect("runtime to include next_line");
        let column = module.get_function("column").expect("runtime to include column");
        let add_file = module.get_function("add_file").expect("runtime to include add_file");
        let init = module.get_function("init").expect("runtime to include init");
        let free_string = module.get_function("free_string").expect("runtime to include free_string");
        let string_to_number = module.get_function("string_to_number").expect("runtime to include string_to_number");
        let number_to_string = module.get_function("number_to_string").expect("runtime to include number_to_string");
        println!("number to string {:?}", number_to_string.get_type());
        println!("get_float {:?}", get_float.get_type());
        println!("get float verify {:?}", get_float.verify(true));
        Self {
            print,
            get_float,
            next_line,
